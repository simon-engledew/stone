...
If you are dealing with large static datasets in Python it can be useful to flyweight your objects. With flyweighting, every time you construct a new object you check to see if it already exists. If so, the original object will be returned instead of constructing a duplicated.
...
On last thing. To get pickle working with flyweighted objects you'll have to create a @__getnewargs__@ method which returns the tuple that will be passed to @__new__@ on unpickling:
...
Although this can also be automated as long as the instance variables are named correctly, giving the final flyweighting class:
...
class FlyweightedObject(object):
    ...
    def __getnewargs__(self):
        tuple(getattr(self, attr) for attr in self.__class__.__init__.im_func.func_code.co_varnames[1:])</code></pre></div>
